...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Employee Management System</title>
        ...
        <div class="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div class="flex justify-between items-center py-6">
                <div class="flex items-center">
                    <h1 class="font-bold text-orange-400 text-2xl">{{ config('app.name', 'Laravel') }}</h1>
                ...
                <nav class="flex items-center space-x-4">
                    ...
                    @else
                    <a href="{{ route('login') }}" class="inline-block px-4 py-2 font-medium text-gray-300 hover:text-white text-sm transition-colors">
                        Sign In
                    </a>
        ...
        <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl">
            <div class="text-center">
                <p class="text-gray-400 text-sm">
                    © {{ date('Y') }} {{ config('app.name', 'Laravel') }} Employee Management System.
                    Built with Laravel & Tailwind CSS.
                </p>
                @auth
                <p class="mt-2 text-gray-500 text-xs">
                    Logged in as {{ auth()->user()->full_name }} ({{ ucfirst(auth()->user()->role) }})
                </p>
                @endauth
            </div>
        </div>
